# Operator guide (business domain)

How to run and steer the business domain. You mostly control it by talking to Claude Code
in plain language. This page lists the few places you touch things directly.

This guide lives at `domains/business/playbooks/operator-guide.md` inside AlphaBrain. Two
repos still apply to the business domain specifically: the public **site** repo
`bamidele-consulting` (deploys to Vercel) and this repo's `domains/business/` (pipeline +
workflow + automation). Edit site code in the site repo; manage content ideas and
automation here.
...
| Content flywheel | How work becomes blog posts and case studies | `content-engine` skill + `domains/business/content/backlog.md` |
| Workflow loop | How the system improves itself | `workflow-scout` skill + the monthly cloud routine |
| Tool/IP scout | How work becomes product ideas | `tool-scout` skill + `domains/business/playbooks/tool-ideas/backlog.md` + the weekly cloud routine |
...
- Capture an idea for later: add a one-line seed to `domains/business/content/backlog.md`.
  The Stop hook also nudges you to do this when business files change.
...
- It opens a daily PR titled "Content radar — <date>", so you get a GitHub notification you
  can read from your phone. The digest is in the PR body and in
  `domains/business/content/radar/<date>.md`.
- The 2-3 strongest picks are added to `domains/business/content/backlog.md`. To act on one, just say
...
- It opens a weekly PR titled "Tool scout — <date>", so you get a GitHub notification you can
  read from your phone. The ranked digest is in the PR body and in
  `domains/business/playbooks/tool-ideas/reports/<date>.md`.
- The top picks are added to `domains/business/playbooks/tool-ideas/backlog.md`. Every candidate is tagged `owned` or
...
| Voice rules | `domains/business/SKILL.md` (and the global memory the skills read) |
